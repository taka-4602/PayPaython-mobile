...
~~IDは```sendbird_group_channel_なんとか_なんとか```の形式だけど```send_message```を使う時の引数にする場合```sendbird_group_channel_```の部分はなくてもOK~~ -> 知らないうちにUUIDと化していた  
#### もう少し効率化
...
-> 予想通り、2026/3/5あたりにBot検知がアップデートされた。最新の対策もまだ公開していません... 
## コンタクト  
